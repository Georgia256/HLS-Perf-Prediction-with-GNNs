...
class PygGraphPropPredDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        additional_node_files_str = str(self.meta_info['additional node files'])  # Convert to string
        if additional_node_files_str.lower() == 'nan':
            additional_node_files = []
        else:
            additional_node_files = additional_node_files_str.split(',')


        additional_edge_files_str = str(self.meta_info['additional edge files'])  # Convert to string
        if additional_edge_files_str.lower() == 'nan':
            additional_edge_files = []
        else:
            additional_edge_files = additional_edge_files_str.split(',')
...
if __name__ == '__main__':
    ...
    pyg_dataset = PygGraphPropPredDataset(name = 'dfg_ff')
    print(pyg_dataset.num_classes)
    print(pyg_dataset[1])
    split_index = pyg_dataset.get_idx_split()
    ...
    loader = DataLoader(pyg_dataset, batch_size=5, shuffle=False)
    for batch in loader:
