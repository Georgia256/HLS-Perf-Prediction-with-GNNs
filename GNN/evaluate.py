from sklearn.metrics import roc_auc_score, average_precision_score, mean_absolute_percentage_error
import pandas as pd
import os
import numpy as np
...
class Evaluator:
    ...
    def _parse_and_check_input(self, input_dict):
        if self.eval_metric == 'rocauc' or self.eval_metric == 'ap' or self.eval_metric == 'rmse' or self.eval_metric == 'acc' or self.eval_metric == 'mape':
    ...
    def eval(self, input_dict):
        ...
        if self.eval_metric == 'ap':
            ...
            y_true, y_pred = self._parse_and_check_input(input_dict)
            return self._eval_mape(y_true, y_pred)
        elif self.eval_metric == 'F1':
    ...
    def expected_output_format(self):
        ...
        if self.eval_metric == 'rocauc':
            ...
            desc += '{\'mape\': mape}\n'
            desc += '- mape (float): Accuracy score averaged across {} task(s)\n'.format(self.num_tasks)
        elif self.eval_metric == 'F1':
            desc += '{\'F1\': F1}\n'
    ...
    def _eval_mape(self, y_true, y_pred):
        ...
        for i in range(y_true.shape[1]):
            ...
            y_true_nonzero = np.where(y_true[:, i] == 0, 1e-5, y_true[:, i])
